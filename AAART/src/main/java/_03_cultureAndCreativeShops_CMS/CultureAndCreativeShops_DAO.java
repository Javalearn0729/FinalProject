...
//	###read shops info 
	...
	public List<CultureAndCreativeShopsTable> searchShops(String str) {
		...
		try {
			...
			PreparedStatement pstmt = connection
					.prepareStatement("SELECT * FROM CS_CULTURE_AND_CREATIVE_SHOPS_TABLE where name = ? ");
	...
	public void updateShopInfo(CultureAndCreativeShopsTable shop) {
		...
		try (Connection connection = getDataSource().getConnection();
				PreparedStatement pstmt = connection.prepareStatement("UPDATE CS_CULTURE_AND_CREATIVE_SHOPS_TABLE SET"
						+ " (group_type_name = ? ,main_type_name = ? ,name = ? ,"
						+ "represent_image = ? ,intro = ? ,  city_name = ?, address = ?,"
...
//			pstmt.setInt(17, Integer.parseInt(shop.getClicks()));
	...
	public void deleteShopsInfo(int id) {

		int shopsMainTypePk = id;
		CultureAndCreativeShopsTable shop= new CultureAndCreativeShopsTable();
		...
		try {
			Connection connection = getDataSource().getConnection();
			PreparedStatement pstmt = connection
					.prepareStatement("DELETE FROM CS_CULTURE_AND_CREATIVE_SHOPS_TABLE where MainTypePk = ? ");

			pstmt.setInt(1, shopsMainTypePk);
			pstmt.executeQuery();
			pstmt.clearParameters();
			pstmt.close();
			connection.close();
